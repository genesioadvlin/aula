...
def playmulti(name, url, iconimage):
        playrun = "plugin://plugin.video.playthis/?mode=play&player=false&history=false&path="+url+""
        PlayUrl(name,playrun,iconimage)

def playmultim3u(name, url, iconimage):
        xbmc.Player().play(""+url+"")


def midia_play(name, url, iconimage):
        tam = len(url)

        if url[tam-3:tam]=="m3u":
                playmultim3u(name, url, iconimage)

        if url[0]=="m":

                keyplay = xbmc.Keyboard('', 'Digite 1 Quasar 2 Pulsar 3 Yatp')
                keyplay.doModal()
                op = keyplay.getText()

                if op ==1:
                        urlt = 'plugin://plugin.video.quasar/play?uri=' + url
                        PlayUrl(name,urlt,iconimage)
                
                if op ==2:
                        urlt = 'plugin://plugin.video.pulsar/play?uri=' + url
                        PlayUrl(name,urlt,iconimage)

                if op ==3:
                        urlt = 'plugin://plugin.video.yatp/?action=play&torrent=' + url
                        PlayUrl(name,urlt,iconimage)

        else:
                playmulti(name, url, iconimage)  
